...
import { EduCoreContextPanel } from "../components/tickets/EduCoreContextPanel";
...
export function TicketDetailsPage() {
  ...
  return <div className="detail-page">
    ...
    {navigationState?.attachmentSuccess && <div className="success-banner" role="status"><Icon name="check" />{navigationState.attachmentSuccess}</div>}
    ...
    {(isTechnician || isAdmin) && <section className="panel action-panel"><div><h3>Support actions</h3><p>Classification suggestions remain visible after an override.</p></div>{isTechnician && <><label><span>Category</span><select disabled={acting || ticket.status === "RESOLVED"} value={ticket.category.id} onChange={(event) => user && act(() => ticketsApi.changeTicketCategory(ticket.id, event.target.value, user))}>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label><label><span>Priority</span><select disabled={acting || ticket.status === "RESOLVED"} value={ticket.priority} onChange={(event) => user && act(() => ticketsApi.changeTicketPriority(ticket.id, event.target.value as TicketPriority, user))}>{(["LOW", "MEDIUM", "HIGH", "URGENT"] as TicketPriority[]).map((priority) => <option key={priority}>{priority}</option>)}</select></label></>}{isAdmin && <label><span>Assigned technician</span><select disabled={acting || ["RESOLVED", "CLOSED", "CANCELLED"].includes(ticket.status)} value={ticket.assignedTechnician?.id ?? ""} onChange={(event) => { const tech = technicians.find((item) => item.id === event.target.value); if (user && tech) act(() => ticketsApi.assignTicket(ticket.id, tech, user)); }}><option value="" disabled>Unassigned</option>{technicians.map((tech) => <option key={tech.id} value={tech.id}>{tech.name}</option>)}</select></label>}</section>}
    <div className="detail-grid"><div className="detail-main">
      <section className="panel detail-section"><h3>Description</h3><p className="description">{ticket.description}</p></section>
